...
static String presentationStateToString(const PresentationState& state) {
	...
	case trialTask:
		...
		break;
	case trialFeedback:
		stateStr = "trialFeedback";
		break;
	case sessionFeedback:
		stateStr = "sessionFeedback";
		break;
	case complete:
		stateStr = "complete";
		break;
...
public:
	...
	shared_ptr<NetworkedSession> netSess = nullptr;		 ///< Pointer to the experiment, as a NetworkedSession so we can avoid doing static_casts.  Watch out, because this only exists when a networked experiment is defined!
