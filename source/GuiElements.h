...
protected:
	...
	RenderControls(FPSciApp* app, SessionConfig& config, bool& drawFps, bool& drawPing, bool& drawLCDebug, const int numReticles, float& brightness,
...
public:
	static shared_ptr<RenderControls> create(FPSciApp* app, SessionConfig& config, bool& drawFps, bool& drawPing, bool& drawLCDebug, const int numReticles, float& brightness,
		const shared_ptr<GuiTheme>& theme, const int maxFrameDelay = 360, const float minFrameRate = 1.0f, const float maxFrameRate=1000.0f, float width = 400.0f, float height = 10.0f) {
		return createShared<RenderControls>(app, config, drawFps, drawPing, drawLCDebug, numReticles, brightness, theme, maxFrameDelay, minFrameRate, maxFrameRate, width, height);
	}
};
